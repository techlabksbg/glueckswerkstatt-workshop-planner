...
# TODO
# Fehler finden, ausgeben und in der Tabelle korrigieren
print("\n\nHier kommen die Fehler\n")
for zeile in tabelle:
    x = zeile['x']
    for exponent in range(2,5):
        korrekt = x ** exponent
        if zeile[f"x^{exponent}"] != korrekt:
            print(f"Fehler bei {zeile} exponent {exponent}")
            print(f"Korrekt ist {korrekt}")
            zeile[f"x^{exponent}"] = korrekt
